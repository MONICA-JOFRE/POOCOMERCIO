<?php
require_once ('personas/Persona.php');
require_once ('personas/Cliente.php');
require_once ('personas/Proveedor.php');
require_once ('personas/Usuario.php');
require_once ('Comercio.php');
require_once ('productos/Producto.php');
require_once ('productos/Factura.php');
require_once ('productos/Detallefactura.php');
require_once ('productos/Pedido.php');
require_once ('productos/Detallepedido.php');